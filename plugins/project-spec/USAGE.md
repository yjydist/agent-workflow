...
`/project-spec:plan-implementation` 只负责生成实现计划并设置 `implementation-planned`. 用户审阅实现计划, 验收标准和必读文档后, 运行 `/project-spec:ready-for-coding` 进入 `ready-for-coding`.
...
- 需求变更使用 `/project-spec:change`.
- 查看状态使用 `/project-spec:status`.
