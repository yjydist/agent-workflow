...
for command in sorted((ROOT / 'commands').rglob('*.md')):
    ...
    expected_title = f'# /project-spec:{command.stem}'
...
if ready_command.exists():
    ...
    for needle in ['# /project-spec:ready-for-coding', '`implementation-planned`', '`ready-for-coding`', 'V1 status', 'Blocking Issues', 'Blocking V1?', '用户确认']:
